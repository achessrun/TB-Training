...
Part 1 has two views, switched at the top of the trial balance section.

- **By lead schedule** groups accounts into the nine audit areas, each with its index
  reference, the reasoning behind the grouping, and the core procedures.
- **Trial balance order** lists all 93 accounts in workbook sequence with a Classification
  column, so you can read straight down the file.

Both views use the workbook's own column layout — 6/30/2025 adjusted, 6/30/2026
unadjusted, AJEs, 6/30/2026 adjusted — and both carry a **Row** column matching the row
numbers in the spreadsheet (rows 10 through 102). The filter box accepts an account name,
an account number, a classification, or a row number.
